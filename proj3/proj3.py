...
st.set_page_config(
page_title = "PSAT Calculator",
page_icon = ":8ball:")
st.caption("By Aarav Arya and Nischal Jain")
st.title("PSAT Calculator")

st.subheader("Enter the input arguments:")
...
with c1:
	# st.write("-"*25)
	constant_fk2d= float(st.number_input('Enter value of constant_fk2d'))
	multiplying_factor_3d=float(st.number_input('Enter value of 3D multiplying factor'))
	Shear_velocity=float(st.number_input('Enter value of Shear Velocity'))

# """ INPUTTING CONSTANT VALUES"""


with c2:
	# """INPUTTING FILTERING METHOD"""
	
	filtering_tuple = ('Select from the dropdown list','C', 'S', 'A','C & S','C & A', 'S & A', 'C & S & A', 'all Combined')
	filtering_method = st.selectbox('Choose filtering method',filtering_tuple)
	fil_meth_code = filtering_tuple.index(filtering_method) 


	# """TAKING INPUT OF C/S/A based on filtering method input"""

	corr, SNR, Lambda, k = 0,0,0,0
	tch = fil_meth_code

	if tch != 0:

		if tch == 1:
			corr = int(st.number_input('Enter thresold value C:', step = 1 ))
		elif tch == 2:
			...
			corr = int(st.number_input('Enter thresold value C:', step = 1 ))
			SNR = int(st.number_input('Enter thresold value S:', step = 1))
		elif tch == 5:
			corr = int(st.number_input('Enter thresold value C:', step = 1 ))
	...
	# Taking Replacement method inputs
with c3:
	replacement_tuple = ('Select from the dropdown list','previous point',' 2*last-2nd_last','overall_mean',
		'12_point_strategy','mean of previous 2 point',
		'all seqential','all parallel')

	replacement_method = st.selectbox('Choose replacement method',replacement_tuple)
sch = replacement_tuple.index(replacement_method) 




if tch == 0 and sch == 0:
	st.warning('To Compute, please choose a filtering method and a replacement method!', icon="⚠️")

elif tch == 0 and sch != 0 :
	st.warning('To  Compute, please choose a filtering method!', icon="⚠️")

elif tch != 0 and sch == 0 :
	st.warning('To Compute, please choose a replacement method!', icon="⚠️")
...
col1, midcol, col2 = st.columns((2,10,2))
with(col2):
	if st.button(":arrows_counterclockwise: Reset"):
